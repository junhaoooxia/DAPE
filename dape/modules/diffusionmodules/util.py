...
class AdjustableNorm(nn.Module):
    """Adjustable normalization layer for DAPE Stage 1 (Adjustable Norm-tuning).

    Implements paper Eq. 3: ẑt = γ·Norm(zt) + β + α0·zt
    where α0 is a learnable scalar initialized to 0, allowing the model to
    start from standard normalization and gradually adapt.
    """

    def __init__(self, norm_layer: nn.Module):
        super().__init__()
        self.norm = norm_layer
        # α0 initialized to 0 so training starts from standard normalization
        self.alpha0 = nn.Parameter(torch.zeros(1))

    def forward(self, x):
        return self.norm(x) + self.alpha0 * x
