...
class StandardDiffusionLoss(nn.Module):
    def __init__(
        ...
        assert type in ["l2", "l1", "lpips", "huber"]
    ...
    def get_loss(self, model_output, target, w):
        if self.type == "l2":
            ...
            # Huber loss: robust to outliers in one-shot fine-tuning (DAPE paper Eq. 5)
            r = (model_output - target).abs()
            delta = self.huber_delta
            loss = torch.where(r <= delta, 0.5 * r ** 2, delta * (r - 0.5 * delta))
            return torch.mean((w * loss).reshape(target.shape[0], -1), 1)