...
class DAPEAdapter(nn.Module):
    def __init__(self,
                 in_dim,
                 inner_dim=64):
        super().__init__()
        # inner_dim = 64 if mlp_ratio is None else int(in_dim * mlp_ratio)
        ...
        self.adapter_conv = nn.Conv2d(inner_dim, inner_dim, kernel_size=5, padding=5 // 2, groups=inner_dim)

        self.norm = nn.LayerNorm(in_dim)
        self.gamma = nn.Parameter(torch.ones(in_dim) * 0.1)
        self.gammax = nn.Parameter(torch.ones(in_dim))

    def forward(self, x, input_hw=None):
        # print('!'*10,input_hw,'!'*10)
        identity = x
        ...
        h, w = input_hw
        assert n == h * w ,'Input height & width error'
        project1 = project1.reshape(b, h, w, c).permute(0, 3, 1, 2)
        identity_project = project1
        project1 = self.adapter_conv(project1) + identity_project
...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if is_PEFT and os.environ.get("DAPE_DISABLE_ADAPTER", "0") != "1":
            self.adapter_dape = DAPEAdapter(dim,inner_dim=64)
            for name,p in self.adapter_dape.named_parameters():
                p.requires_grad = False
        if self.checkpoint:
            print(f"{self.__class__.__name__} is using checkpointing")

    def forward(
        self, x, context=None, additional_tokens=None, n_times_crossframe_attn_in_self=0,input_hw: torch.Tensor = None
    ):
        kwargs = {"x": x}
        ...
        return checkpoint(
            self._forward, (x, context,input_hw), self.parameters(), self.checkpoint
        )

    def _forward(
        self, x, context=None,input_hw: torch.Tensor = None, additional_tokens=None, n_times_crossframe_attn_in_self=0
    ):
        x = (
            self.attn1(
                self.norm1(x),
        ...
        if hasattr(self, 'adapter_dape') and input_hw is not None:
            h, w = int(input_hw[0].item()), int(input_hw[1].item())
            if x.shape[1] == h * w and any(p.requires_grad for p in self.adapter_dape.parameters()):
                x = self.adapter_dape(x, input_hw=(h, w))
        x = (
            self.attn2(
...
class BasicTransformerSingleLayerBlock(nn.Module):
    ...
    def __init__(
        ...
        if is_PEFT and os.environ.get("DAPE_DISABLE_ADAPTER", "0") != "1":
            ...
            for name,p in self.adapter_dape.named_parameters():
                p.requires_grad = False
                # print(f"{name}: {p.requires_grad}")  # confirm parameter setup is correct

    def forward(self, x, context=None,input_hw: torch.Tensor = None):
        return checkpoint(
            self._forward, (x, context,input_hw), self.parameters(), self.checkpoint
        )

    def _forward(self, x, context=None,input_hw: torch.Tensor = None):
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x, context=None):
        ...
        if self.use_linear:
            x = self.proj_in(x)
        input_hw = torch.tensor([h, w], dtype=torch.float16,device=x.device,requires_grad=False)
        for i, block in enumerate(self.transformer_blocks):
            if i > 0 and len(context) == 1:
                i = 0  # use same context for each block
            if self.disable_text_ca:
                x = block(x, context=x, input_hw = input_hw)
            else:
                x = block(x, context=context[i], input_hw = input_hw)
        if self.use_linear:
            x = self.proj_out(x)
...
class SpatialTransformerCA(SpatialTransformer):
    ...
    def forward(self, x, context=None):
        ...
        for i, block in enumerate(self.transformer_blocks_ca):
            ...
            x = block(x, context_texture,input_hw = input_hw)
...
class SpatialTransformer3D(nn.Module):
    ...
    def forward(self, x, context=None):
        ...
        for i, block in enumerate(self.transformer_blocks):
            ...
            x = block(x, context=context_i,input_hw = input_hw)
        if self.use_linear:
            x = self.proj_out(x)
    ...
    def forward(self, x, context=None):
        ...
        for i, block in enumerate(self.transformer_blocks_temporal_ca):
            ...
            if self.ST3DCA_ca_type == 'center':
                x = block(x, context=context_texture,input_hw = input_hw)
