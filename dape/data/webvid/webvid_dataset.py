...
class SingleVideoDataset(TextVideoDataset):
    """
    Single Video Dataset: repeats one video for training.
    Expects video_params to contain 'video_path', 'caption', and optionally 'repeat'.
    """

    def _load_metadata(self):
        video_path = self.video_params.get("video_path", "")
        caption = self.video_params.get("caption", "")
        self.metadata = pd.DataFrame({
            "video_path": [video_path],
            "caption": [caption],
        })
        self._repeat = self.video_params.get("repeat", 1000)

    def __len__(self):
        return self._repeat

    def _get_video_path(self, sample):
        full_video_fp = sample["video_path"]
        rel_video_fp = os.path.basename(full_video_fp)
        return full_video_fp, rel_video_fp
