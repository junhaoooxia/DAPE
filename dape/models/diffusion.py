...
from dape.modules.encoders.modules import VAEEmbedder
from dape.modules.encoders.modules import (
    LineartEncoder,
    DepthZoeEncoder,
    DepthMidasEncoder,
    SoftEdgeEncoder,
...
class DiffusionEngine(pl.LightningModule):
    ...
    def init_from_ckpt(
        ...
        # DAPE: key remap (inherited from CCEdit's model_load_ckpt). The CCEdit
        # checkpoint stores VAE weights under conditioner.embedders.*.first_stage_model.*
        # (because VAEEmbedder references the same first_stage_model). Remap them
        # to first_stage_model.* so the VAE loads correctly instead of using
        # random init (which produces garbled color blocks at decode).
        sd_new = {}
        for k, v in sd.items():
...
class VideoDiffusionEngineTV2V(VideoDiffusionEngine):
    def __init__(self, *args, **kwargs):
        ...
        if hasattr(self.model.diffusion_model, "controlnet_img"):
            print('Setting controlnet_img to untrainable ... ')
    ...
    def _replace_norms_with_adjustable(self):
        """Replace all norm layers in the diffusion model with AdjustableNorm.

        Implements DAPE Stage 1 (Adjustable Norm-tuning): ẑt = γ·Norm(zt) + β + α0·zt
        α0 is initialized to 0 so training starts from standard normalization.
        """
        from dape.modules.diffusionmodules.util import AdjustableNorm

        device = next(self.model.diffusion_model.parameters()).device

        def _replace_recursive(module):
            for name, child in module.named_children():
                if isinstance(child, (nn.GroupNorm, nn.LayerNorm)) and not isinstance(child, AdjustableNorm):
                    setattr(module, name, AdjustableNorm(child).to(device))
                else:
                    _replace_recursive(child)

        _replace_recursive(self.model.diffusion_model)
        print(f"Replaced all norm layers with AdjustableNorm for Stage 1 norm-tuning (device={device}).")
    ...
    def configure_optimizers(self):
                ...
                for name, param in self.model.diffusion_model.named_parameters():
                    if "controlnet" not in name:
                        if "temporal" in name:
                            ...
                            param.requires_grad = False
            elif self.freeze_model == 'normtuning':
                # DAPE Stage 1: jointly train norm (γ, β) + adapter_mona
                params = []
                for name, param in self.model.diffusion_model.named_parameters():
                    param.requires_grad = False
                    if "controlnet" not in name:
                        if "adapter_mona" in name:
                            ...
                            params.append(param)
                            param.requires_grad = True
                print(f'normtuning training params num: {len(params)}')
            else:
                raise NotImplementedError
        ...
        if self.stage == 2:
            ...
            for name, param in self.model.diffusion_model.named_parameters():
                if "adapter_mona" in name:
                    params.append(param)
                    # print(f'adapter_mona params name:{name}')
                else:
                    param.requires_grad = False
            print(f'adapter_mona params name:{len(params)}')
            # raise KeyError
            ...
            for embedder in self.conditioner.embedders:
                if embedder.is_trainable:
                    params = params + list(embedder.parameters())
            opt = self.instantiate_optimizer_from_config(params, lr, self.optimizer_config)
            # Stage 2 uses constant lr=1e-5 (no warmup scheduler) — with only
            # ~70 steps, the Stage 1 scheduler's 100-step warmup would leave
            # Stage 2 never reaching target lr.
            print(f"Stage 2: constant lr={lr}, no scheduler")
            return opt
...
class VideoDiffusionEngineTV2VInterpolate(VideoDiffusionEngineTV2V):
    ...
    def log_images(
        ...
        **kwargs,
        ...
        from dape.modules.encoders.modules import CustomIdentityEncoder, CustomIdentityDownCondEncoder
