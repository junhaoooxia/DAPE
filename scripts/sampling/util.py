...
from dape.modules.diffusionmodules.sampling import (
    DPMPP2MSampler,
...
from dape.util import exists, instantiate_from_config, isheatmap
...
def init_sampling(
    ...
    guider_config_target="dape.modules.diffusionmodules.guiders.VanillaCFG",
    cfg_scale=7.5,
...
def get_discretization(discretization):
    if discretization == "LegacyDDPMDiscretization":
        discretization_config = {
            "target": "dape.modules.diffusionmodules.discretizer.LegacyDDPMDiscretization",
        }
        ...
        discretization_config = {
            "target": "dape.modules.diffusionmodules.discretizer.EDMDiscretization",
            "params": {
                "sigma_min": sigma_min,
                "sigma_max": sigma_max,
                "rho": rho,
...
def get_guider(
    ...
    if guider == "IdentityGuider":
        guider_config = {
            "target": "dape.modules.diffusionmodules.guiders.IdentityGuider"
        }
        ...
        if thresholder == "None":
            dyn_thresh_config = {
                "target": "dape.modules.diffusionmodules.sampling_utils.NoDynamicThresholding"
            }
        else:
...
def loadmp4_and_convert_to_numpy(file_path):
    ...
    Returns:
        frames (numpy.ndarray): A numpy array of frames, shape (T, H, W, C).
    """
    ...
    # Handle both decord native NDArray (has .asnumpy()) and torch tensors (has .numpy())
